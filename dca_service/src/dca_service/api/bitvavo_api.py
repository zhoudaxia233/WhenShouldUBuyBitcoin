...
from dca_service.models import BitvavoCredentials, User
...
router = APIRouter(prefix="/bitvavo", tags=["bitvavo"])
...
def save_credentials(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Unable to secure Bitvavo credentials") from e

    existing = session.exec(
        select(BitvavoCredentials).where(BitvavoCredentials.credential_type == creds.credential_type)
    ).first()
    if existing:
    ...
    session.commit()
    return {"success": True, "message": f"Bitvavo {creds.credential_type} credentials saved."}
...
def get_credentials_status(
    ...
    creds = session.exec(
        select(BitvavoCredentials).where(BitvavoCredentials.credential_type == credential_type)
    ).first()
    if not creds:
        return CredentialsStatus(has_credentials=False)
    try:
        plain_key = decrypt_text(creds.api_key_encrypted)
        masked = f"{plain_key[:3]}****{plain_key[-4:]}" if len(plain_key) > 8 else "****"
    except Exception:
        masked = "ERROR"
...
async def test_connection(
    ...
    try:
        ...
        return ConnectionTestResult(success=False, error_message="Bitvavo connection test failed")
    finally:
...
async def get_trading_status(
    ...
    creds = session.exec(
        select(BitvavoCredentials).where(BitvavoCredentials.credential_type == "TRADING")
    ).first()
    if not creds:
        return TradingStatus(
            ...
            error_message="No Bitvavo trading credentials configured",
        )
    client = None
    try:
        client = BitvavoClient(
        ...
        await client.test_trading_permission(get_exchange_symbol("BITVAVO"))
        return TradingStatus(
            has_credentials=True,
        ...
        return TradingStatus(
            ...
            has_trading_permission=False,
            can_enable_live=False,
            error_message="Bitvavo trading connection failed",
        )
    finally:
        if client is not None:
            try:
