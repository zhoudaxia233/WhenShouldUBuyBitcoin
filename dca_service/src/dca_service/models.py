...
class DCAStrategy(SQLModel, table=True):
    ...
    enforce_monthly_cap: bool = Field(default=True)  # If True, enforce strict monthly budget limit with resets
    accumulated_savings: float = Field(default=0.0)  # Available savings (monthly budget or accumulated funds)
    last_monthly_inflow: Optional[datetime] = Field(default=None)  # When we last added budget to savings
